...
Running `make dev` today opens the default `create-tauri-app` scaffold page (framework logos, a "Greet" button) — M1 (Import → Library → Basic Develop → Export) hasn't been built yet, so there's no real product UI to try. What *is* real in this build:

- **The Rust core** (catalog schema, RAW decoding) — exercised via tests, not the UI yet: `make test`.
- **The M0 WebGPU validation page** — the actual proof that the rendering architecture works, at `app/src/routes/m0-spike/`. Run `make spike` to load it directly (auto-reverts the config change on exit).
...
Developed and validated on **macOS only** so far. Two concrete (not hypothetical) Windows gaps are already known and tracked in [PROGRESS.md](PROGRESS.md):

- `rsraw` (RAW decoding) doesn't build under the MSVC toolchain Windows Tauri builds default to.
- WebView2's WebGPU support hasn't been tested.

Windows work is deferred by design, not forgotten — see [PROGRESS.md](PROGRESS.md) before starting a Windows build.

## Working practices

- `main` is the stable branch. Non-trivial work happens on `feature/*` branches with logically-grouped commits, then merges to `main` — see the commit history for the pattern.
