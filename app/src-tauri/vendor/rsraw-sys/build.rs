...
fn build(out_dir: impl AsRef<Path>) {
    ...
    if compiler.is_like_msvc() {
        // Upstream panics here ("MSVC is not supported") -- compiling this
        // vendored LibRaw source tree directly under MSVC hits real
        // incompatibilities. Instead, link a prebuilt LibRaw installed via
        // vcpkg (see ../PATCH.md and docs/adr/ADR-0003-raw-decoding.md).
        // vcpkg::find_package emits the cargo:rustc-link-* directives
        // itself, so there's nothing else to do on this path.
        vcpkg::find_package("libraw").expect(
            "vcpkg could not find libraw -- install it first: \
