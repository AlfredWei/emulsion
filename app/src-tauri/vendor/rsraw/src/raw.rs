...
impl RawImage {
    ...
    /// the "raw scene radiance" mode multi-exposure HDR merging needs, as
    /// opposed to the display-referred, auto-exposed output every other
    /// consumer of this crate wants. `gamm = [1.0, 1.0, ...]` disables
    /// LibRaw's gamma/tone curve (linear output); `no_auto_bright`
    /// disables LibRaw's own per-image auto-exposure compensation, which
    /// would otherwise re-normalize each frame independently and defeat a
    /// caller's own controlled exposure-ratio scaling across frames. Call
    /// before `process::<BIT_DEPTH_16>()` (8-bit linear would quantize away
    /// almost all of the dynamic range this mode exists to preserve, so
    /// this is meant to be paired with 16-bit output, not enforced here).
