...
export function writeAdjustmentsAndRender(/** @type {import('./gpuHandles.js').GpuHandles} */ gpu, /** @type {import('./gpuHandles.js').RenderInputs} */ inputs) {
  ...
  if (spatialOpsKey !== gpu.spatialOpsInputsKey) {
    ...
    // Luma NR's guided filter (RFC-0012): 11 passes replacing the old
    // lumaNRHPipeline/lumaNRVPipeline pair -- same shape as Clarity's own
    // 10-pass block above (mean_p, corr_p, a, b, mean_a, mean_b, final),
    // ported at Luma NR's own radius/eps, still reading gradedTex directly
    // (never rebound). lumaNRBlurHTex is reused as the shared H-scratch
    // for all four box-filter pairs here, same rebinding trick as above.
    runFullscreenPass(encoder, gpu.lumaNRMeanpHPipeline, gpu.lumaNRMeanpHBindGroup, gpu.lumaNRBlurHTex.createView());
    runFullscreenPass(encoder, gpu.lumaNRMeanpVPipeline, gpu.lumaNRMeanpVBindGroup, gpu.lumaNRMeanPTex.createView());
    runFullscreenPass(encoder, gpu.lumaNRCorrpHPipeline, gpu.lumaNRCorrpHBindGroup, gpu.lumaNRBlurHTex.createView());
    runFullscreenPass(encoder, gpu.lumaNRCorrpVPipeline, gpu.lumaNRCorrpVBindGroup, gpu.lumaNRCorrPTex.createView());
    runFullscreenPass(encoder, gpu.lumaNRAPipeline, gpu.lumaNRABindGroup, gpu.lumaNRATex.createView());
    runFullscreenPass(encoder, gpu.lumaNRBPipeline, gpu.lumaNRBBindGroup, gpu.lumaNRBTex.createView());
    runFullscreenPass(encoder, gpu.lumaNRMeanaHPipeline, gpu.lumaNRMeanaHBindGroup, gpu.lumaNRBlurHTex.createView());
    runFullscreenPass(encoder, gpu.lumaNRMeanaVPipeline, gpu.lumaNRMeanaVBindGroup, gpu.lumaNRMeanATex.createView());
    runFullscreenPass(encoder, gpu.lumaNRMeanbHPipeline, gpu.lumaNRMeanbHBindGroup, gpu.lumaNRBlurHTex.createView());
    runFullscreenPass(encoder, gpu.lumaNRMeanbVPipeline, gpu.lumaNRMeanbVBindGroup, gpu.lumaNRMeanBTex.createView());
    runFullscreenPass(encoder, gpu.lumaNRFinalPipeline, gpu.lumaNRFinalBindGroup, gpu.lumaNRBlurTex.createView());
