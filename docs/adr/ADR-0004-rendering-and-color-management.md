...
## Update — M5 Slice 3: interactive slider-latency budget measured for the first time (2026-09-03/04)

This ADR's central premise — keeping the interactive edit loop entirely in-webview specifically to hit the PRD's ≤100ms slider-feedback budget — had never actually been measured against a live render. M5 Slices 1 and 2 both built safety nets *around* this render path (CPU fallback, CPU/GPU parity) without timing it. This slice measured it directly, via a new `recordRenderLatency` instrumentation point in `DevelopCanvas.svelte`'s render effect (timed from the effect's own fire to a real `device.queue.onSubmittedWorkDone()` completion, not a JS-return-time proxy) and a new `e2e/specs/develop-performance.e2e.js` that drives five representative sliders spanning this shader's cost range.

- **Real numbers, this dev machine (current-generation, not the PRD's 5-year-old mid-range target)**: Exposure 25ms, Contrast 22ms, HSL green-band saturation 20ms, Sharpen (separable H+V blur) 3ms, Luminance NR (separable H+V blur) 2ms — all comfortably under the 100ms budget, with the two heaviest-looking passes (the separable blurs) actually the *cheapest* measured, likely a GPU/driver warm-up effect from being later in the accumulating scenario sequence rather than a property of the passes themselves; not chased further here since it doesn't threaten the budget in either direction. A pass on current hardware is real, useful evidence that this architecture has headroom — it is not itself confirmation of the PRD's actual 5-year-old-mid-range target, which this environment cannot test.
- **A real, previously-undiscovered SQLite performance bug was found and fixed while validating the other half of MILESTONES.md's M5 exit criterion** ("...on a 50k-image catalog..."). The interactive render loop itself is architecturally decoupled from catalog size by this ADR's own design (it never touches the catalog), so the actual catalog-scale risk lives in the persistence path instead: `catalog.rs`'s `list_images()` — the Library grid's own full-catalog query — had no index on `image_versions.image_id`, the column its own per-row correlated subquery filters on. Confirmed via `EXPLAIN QUERY PLAN` (`SCAN image_versions` before, `SEARCH ... USING INDEX` after) and directly measured: 37.4s (release build) for one `list_images()` call over a real 50,000-image catalog before the fix, ~75ms after adding `idx_image_versions_image_id`. New `#[ignore]`-gated `catalog_scales_to_50k_images` test (run explicitly, not part of the default fast suite) is the permanent regression guard. Full detail in `catalog.rs`'s own comments at the index and the test.

**Consequence carried forward**: Windows/WebView2 hardware validation remains the only unaddressed real risk this ADR still names — untouched by this slice, same as Slice 1's own note above.
