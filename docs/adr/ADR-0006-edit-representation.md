...
## Update — a `panel_hidden` marker op, presence-means-hidden (2026-09-24, RFC-0013)

RFC-0013 adds one new, minimal op kind: `{op: "panel_hidden", panel: "<key>"}`, backing each of the 12 op-bearing Develop panels' own eye-icon "visible" toggle (real Lightroom's panel switch). Unlike every other op this ADR governs, it carries no adjustment value of its own — its *presence* in the stack is the entire payload, meaning "treat every op belonging to `panel` as absent for rendering purposes, without deleting or altering those ops' own stored entries." This is a direct extension of this ADR's own established "absence of an op = its identity default" convention (already relied on by `resetEditStack` and every op's own getter), applied one level up: a *hidden panel's* ops become absent-as-far-as-rendering-is-concerned, while remaining fully present, stored, and user-editable in the stack itself.

**Where it's interpreted**: not inside `apply_edit_stack`/`apply_lens_correction`/`apply_perspective`/`apply_crop` themselves (ADR-0004's CPU reference implementation) — those four functions stay completely unaware this feature exists. Instead, a new `effective_stack_for_render` helper strips hidden panels' ops (and the `panel_hidden` markers themselves) once, at each of the three real render entry points (`preview_cache.rs`, `export.rs`, `import.rs`'s thumbnail regeneration) — so preview, export, and thumbnails all agree, and this persists with the photo (affects export), not merely an on-screen preview toggle. The GPU/WGSL side needed no shader or uniform-buffer change at all: `developView.svelte.js` gained a parallel `render*` field per adjustment (reading through the JS twin of the same filter) specifically for what `DevelopCanvas` consumes, while the original UI-facing fields keep reading the raw stack unfiltered — a hidden panel's sliders stay visible and editable (dimmed, not reset), matching real Lightroom's own panel-switch behavior, which the render-facing split exists specifically to preserve.

**Reset is a separate, independent op-removal action** (existing per-panel op names filtered out of the stack, same mechanism `resetEditStack` already used for the whole stack), not part of this update's own schema — it never touches a panel's `panel_hidden` marker, and toggling visibility never touches a panel's stored values.
