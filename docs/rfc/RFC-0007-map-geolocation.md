...
**Map UX polish (2026-09-22), on top of slices 2b/3a.** Three usability changes to the same view, all requested after the RFC's three slices had already shipped -- the last one replaces part of what 3a built, so it's described as it now stands rather than layered on top.

- **Pin thumbnails:** a single-photo pin shows that photo's thumbnail once one exists (generated on the spot via `ensureThumbnail` if it doesn't, the same lazy fetch Grid/Loupe/Filmstrip already use, deduplicated per photo per time the map is open), instead of a plain dot.
- **Cluster look:** a cluster doesn't show one representative photo standing in for the whole group either. It fans up to three of its own photos like a hand of cards (back layers rotated/offset/dimmed for depth), count badged in the corner. Each layer requests its own thumbnail independently and fills in as it arrives; the shape is the same at every zoom, since it's `clusterPoints` that changes per zoom, not this rendering.
- **Placing is drag-and-drop only, replacing 3a's click-to-drop-a-pin-then-Apply flow.** Filmstrip cells are drag sources (dragging a cell that's part of the current multi-selection carries the whole selection, by `image_id`, de-duplicating virtual copies) and `LibraryMapView` the drop target: a pin follows the pointer while dragging, at the exact point a drop would use, and letting go commits that point through `handleMapAssignLocation` immediately -- no confirm step. This exists specifically because the Filmstrip is the *only* photo strip still visible while the Map is Library's active view (Grid, the natural first choice, is not, since the two are mutually exclusive Library view modes) -- there was otherwise nowhere to drag *from*.
- **This is plain pointer events, not native HTML5 drag-and-drop -- a first version used the latter and it did not work in the actual app.** The app already relies on Tauri's own window-level drag-drop interception for file import (`onDragDropEvent` in `appEvents.js`, needed because a webview's own File API can't give a real path on every platform); once that's active it stops the browser's native `dragstart`/`dragover`/`drop` DOM events from firing at all, even for a drag that never leaves the page. A native-DnD version of Filmstrip→map therefore looked fine in the plain dev-server browser (no such interception there) and silently did nothing in the built app -- a real report, and a genuine miss, since only the browser check the first pass had access to. Rebuilt on plain pointer events (`shell.photoDrag`, `shell.svelte.js`), the same pattern this codebase already uses for every other drag interaction (`DevelopCanvas`'s crop handles, the Develop panel-resize rails), which never touches native drag-and-drop and so isn't affected by that interception at all: armed on pointerdown, becomes a real drag only past a small movement threshold (a plain click still just selects), and `LibraryMapView` reacts to the shared state with a `$effect` that does its own hit-testing against its own bounding rect on every pointer update -- the same shape a native dragover/dragleave/drop trio would have had, just driven by a store instead of the DOM's own drag events.
