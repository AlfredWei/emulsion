...
- **Placing is drag-and-drop only, replacing 3a's click-to-drop-a-pin-then-Apply flow.** Filmstrip cells are drag sources (dragging a cell that's part of the current multi-selection carries the whole selection, by `image_id`, de-duplicating virtual copies) and `LibraryMapView` a drop target: a pin follows the pointer while dragging, at the exact point a drop would use (computed from the same `container`-relative pixels `handleDrop` uses, so the preview and the save are always consistent), and dropping calls `handleMapAssignLocation` immediately -- no confirm step, the drop is the confirmation. This exists specifically because the Filmstrip is the *only* photo strip still visible while the Map is Library's active view (Grid, the natural first choice, is not, since the two are mutually exclusive Library view modes) -- there was otherwise nowhere to drag *from*.
- **The drag needed its own MIME kept out of `LibraryModule`'s import handling.** A photo drag and an OS file drag (for importing) can both land on the Library body, and the first pass conflated them: dragging a Filmstrip cell across the rail on the way to the map set off `LibraryModule`'s "drop files to import" overlay (nothing on that path reset it), which then sat on top of the map for the rest of the drag even though the map's own handling kept working underneath. Fixed by giving the photo drag its own MIME, in a small shared module (`lib/dragTransfer.js`'s `IMAGE_IDS_DRAG_MIME`, imported by `Filmstrip`, `LibraryMapView`, and `LibraryModule` alike) so `LibraryModule`'s import handlers can check for it and ignore that drag outright, regardless of which part of the body the pointer is over when a `dragover` fires. `LibraryMapView` additionally `stopPropagation`s on drags it claims, as defense in depth.
