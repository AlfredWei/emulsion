...
**Pin thumbnails (2026-09-22), a small addition on top of slice 2b.** Each pin/cluster now shows a thumbnail of a representative photo (the same first-seen photo its cluster is keyed to) instead of a plain dot once one exists, requested on the spot (`ensureThumbnail`, the same lazy fetch Grid/Loupe/Filmstrip already use, deduplicated per photo per time the map is open) when it doesn't. A cluster keeps its numbered badge, now overlaid on the corner of the thumbnail rather than being the whole pin. Falls back to the original plain dot/count until a thumbnail exists, so nothing regresses for a catalog that hasn't generated thumbnails yet.
