...
- **Placing is drag-and-drop only, replacing 3a's click-to-drop-a-pin-then-Apply flow.** Filmstrip cells are drag sources (a custom `application/x-emulsion-image-ids` MIME; dragging a cell that's part of the current multi-selection carries the whole selection, by `image_id`, de-duplicating virtual copies) and `LibraryMapView` a drop target: a pin follows the pointer while dragging, at the exact point a drop would use (computed from the same `container`-relative pixels `handleDrop` uses, so the preview and the save are always consistent), and dropping calls `handleMapAssignLocation` immediately -- no confirm step, the drop is the confirmation. This exists specifically because the Filmstrip is the *only* photo strip still visible while the Map is Library's active view (Grid, the natural first choice, is not, since the two are mutually exclusive Library view modes) -- there was otherwise nowhere to drag *from*. A drag not carrying that MIME (a real OS file drop) is left alone and still bubbles up to `LibraryModule`'s existing import-by-drop handling, via `stopPropagation` only on drags this view claims.
