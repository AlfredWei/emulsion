...
| `masks.svelte.js` | `activeTool`, `selectedMaskId`, brush settings, `spotBrushSize`, `maskOverlaysVisible`, `showMaskOverlay`, `eyedropperTarget`, `colorRangeResampleTarget`; derived `masks`, `selectedMask`, `isResamplingColor` | **As built (P6a):** `MaskStore(develop)`; the derived list is `masks.list` (not `masks.masks`). `install()` holds the two self-cleaning effects. Mask workflows are `actions/maskActions.js` (P6b). |
| `softProof.svelte.js` | the `softProof*` settings, `softProofPreviewUrl`, `softProofLoading`, `softProofTimer`; derived `softProofProfileLabel` | **As built (P6a):** `SoftProofStore(develop)`, fields drop the `softProof` prefix (`softProof.enabled`, `.target`, `.previewUrl`, `.profileLabel`); the timer is `#private` and `install()` holds the preview effect. |
| `presets.svelte.js` | `presets`, `creatingPreset`, `confirmingDeletePresetId`, `applyingPreset`, `creatingSnapshot`, `confirmingReset`, `copySettingsDialogOpen`, `pastingSettingsToSelection` | **As built (P6a):** state only; the list is `presets.list`. Refresh/create/delete/apply and paste-settings are `actions/presetActions.js` (P6b). |
...
| `exportFlow.svelte.js` | `items` (the Export dialog's list; `null` = closed); derived `currentItems` (was `currentExportItems`) | **As built (P6c):** `ExportFlowStore(shell, develop, selection)`; the store DAG gains exportFlow → shell, develop, selection, and `print` no longer needs to reach `currentExportItems` (navigation copies it into `print.items` on entry). This is the resolution of the note on the `print` row. |
...
| stays in the component that binds it | `imageViewerRef` (`bind:this`) → `LibraryModule` | **As built (P8):** it is local to `LibraryModule` and needs no ctx getter: only the Library toolbar's zoom-fit / 100% buttons read it, and `keyboard.js` never did. |
...
|---|---|
| `actions/navigation.js` | `openDevelop`, `switchModule` (**as built (P6c):** plus `selectNextImage`/`selectPrevImage` and `handleExportClick`) |
...
| **P6** | `masks`, `softProof`, `presets` (+`presetActions.js`), with their `install()` effects | medium. *As split:* **P6a** the three stores + effects (merged); **P6b** `maskActions`, `presetActions`, `softProofActions`, `historyActions` (merged); **P6c** `exportFlow` store and `navigation.js` (`openDevelop`, `switchModule`, next/prev image, export click). |
| **P7** | `appEvents.js` (the `onMount` body) | medium — window-close flush |
| **P8** | `LibraryModule`, `DevelopModule`, `PrintModule` components; scoped CSS moves with markup | high — e2e DOM shape. **As built:** three prop-less components in `lib/components/` that import the stores and actions they use; the page keeps the titlebar, status strip, the `{#if}` that picks the module, the Develop info bar and both Filmstrips (they must also render in the "Develop with no image open" placeholder state). The shared `.body/.develop-body/.print-body` CSS rule is split into one rule per component. The rendered DOM was verified identical (below). |

**Outcome (P8):** `+page.svelte` is 381 lines (script 242, of which about 110 are imports; template 123; style 15), down from 3,665 before P4a and 4,788 at the first snapshot taken for this refactor. The DOM check for the four sensitivities in §5.3 was made stronger than "must exist": the page was rendered to HTML (Svelte SSR) in 17 store scenarios before and after, and every scenario is identical once scoped-CSS hash classes and comment anchors are stripped.
